...
import { generateWeeklyContentPlan } from '../services/social'
...
async function generateMorningBriefing(businessId: string, business: any): Promise<string> {
...
// ============================================
export const invoiceReminders = functions.pubsub
  .schedule('0 10 * * *')
  .timeZone('America/New_York')
  .onRun(async () => {
...
// ============================================
export const scheduledPosts = functions.pubsub
  .schedule('*/15 * * * *')
  .onRun(async () => {
    const now = new Date()
...
// ============================================
export const weeklyContentPlan = functions.pubsub
  .schedule('0 18 * * 0')
  .timeZone('America/New_York')
  .onRun(async () => {
    const businessesSnap = await db
    ...
    for (const businessDoc of businessesSnap.docs) {
      ...
      try {
        const contentPlan = await generateWeeklyContentPlan(businessId)
...
// ============================================
export const pollEngagement = functions.pubsub
  .schedule('0 */6 * * *')
  .onRun(async () => {
...
function calculateEngagementRate(engagement: any): number {
  if (!engagement.reach || engagement.reach === 0) return 0
