...
export async function handleSocialPost(
  ...
  message: string,
  mediaUrls: string[],
  intent: 'social_post' | 'social_schedule'
  ...
  const hasVideo = mediaUrls.some((url) =>
    /\.(mp4|mov|avi|webm)$/i.test(url) || url.includes('video')
  )
  ...
  const posts = await generateMultiPlatformPosts(
    business,
    message,
    mediaUrls,
    platforms,
...
function detectTargetPlatforms(message: string, hasVideo: boolean): string[] {
...
async function generateMultiPlatformPosts(
  business: any,
  message: string,
  mediaUrls: string[],
  platforms: string[],
...
function parseScheduleTime(message: string): Date | null {
  const now = new Date()
  const lower = message.toLowerCase()
...
// ENGAGEMENT TRACKING
// ============================================
export async function trackEngagement(businessId: string, postId: string, platform: string) {
...
// ============================================
export async function generateWeeklyContentPlan(businessId: string): Promise<string> {
  const businessDoc = await db.collection('businesses').doc(businessId).get()
  const business = businessDoc.data()
...
export async function getBestPostingTime(
  ...
  if (engagementSnap.empty) {
    ...
    const defaults: Record<string, { day: string; hour: number }> = {
